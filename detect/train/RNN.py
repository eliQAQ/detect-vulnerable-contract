...
model.add(LSTM(32, activation='sigmoid', dropout=0.55))
...
history = model.fit(X_train_res, ytrainres_cat, epochs=30, batch_size=64, validation_data=(X_val_res, yvalres_cat),
...
y_pred = model.predict(X_test_res, batch_size=64, verbose=0, callbacks=[tensorboard_callback])
...
identifier = 'contrastv3_150batch_size32KLSTM64epoch100_train0.6_2_dimension_data3_5_13_1' + datetime.now().strftime(
    "%Y%m%d-%H%M%S")
