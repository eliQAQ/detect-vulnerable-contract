...
class State:
    ...
    # state记录包含当前状态跳转的下一个基本块，栈，存储空间，和记录从call到storage的序列
...
class OperandStackElement:
    ...
    def __init__(self, pc, instruction, value=None, source=None):
        ...
        self.source = source  # Source PUSH producing stack value #溯源该值是怎么push进来的
...
def backtrack_instruction_sources(pc, instr, operands, ins):
    ...
    # 如果操作码是swap或者dup，则标记存储标志，并不考虑其数据流
    if opcodes.is_swap(op) or opcodes.is_dup(op):
        ...
        # 记录当前pc下，栈中存储的指令跟哪些pc有关
        for i in range(ins):
            dfg.add_edge(operands[i].pc, pc)


def advance(elm, free_storage):
    """
    ...
    global trace
    stat = elm.state
    blk = stat.next_block  # 当前状态的下一个基本块
    pc = blk.offset
    ...
    for instr in blk.instructions:
        ...
        if name == 'STOP' or name == 'SUICIDE':
            ...
            # Can not resolve read address or read size, record all possible memory dependencies
                ...
                # 当无法计算阅读的地址和大小时，则对所有可能的memory建立依赖
                for addr in memory:
            ...
            if dst_pc not in contr.jump_destination:
                ...
                # 建立一个block之间的连接
                cfg.add_edge(blk.offset, dst_blk.offset, type='JUMP')
            ...
            if dst_pc in contr.jump_destination:
                dst_blk = contr.jump_destination[dst_pc]
                # Add CFG edges, representing control flow dependencies between basic blocks
                cfg.add_edge(blk.offset, dst_blk.offset, type='JUMPI')
                # Update number of return values to wait
                elm.num += 1
                ret.append(State(dst_blk, stack, memory, storage, sequence))
            if blk.next is not None:
                # Add CFG edges, representing control flow dependencies between basic blocks
                cfg.add_edge(blk.offset, blk.next.offset, type='JUMPI')
                # Update number of return values to wait
                elm.num += 1
    ...
    # 没有跳转就直接到下一个相连的基本块来判断其状态
    if blk.next is not None:
        # Add CFG edges, representing control flow dependencies between basic blocks
        cfg.add_edge(blk.offset, blk.next.offset, type='SEQUENTIAL')
        # Update number of return values to wait
        elm.num += 1
...
def execute(_contr, _dfg, _cfg, _trace, free_storage):
    ...
    # executionstackelement包含是否执行，当前状态，和返回值
    exec_stack = [ExecutionStackElement(init_stat)]
    ret_stack = []
    vis_stats = {init_stat}

    # ret_stack涉及的判断只有在判断state中的next_block.revert才有用
    while len(exec_stack) > 0:
        ...
        if cur_elm.executed:
            ...
            if cur_elm.num == 0:
                ...
                # cur_elm.num == 1说明只被栈内一个元素影响
                # revert的状态为ret_stack的状态,该基本块是否需要revert取决于前一个块是否需要revert
        ...
        # Current state has NOT been executed, execute it
            ...
            # 执行栈还未执行A
            ret = advance(cur_elm, free_storage)
