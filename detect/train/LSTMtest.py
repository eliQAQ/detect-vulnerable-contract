...
def XandY(posdf, negdf):
    ...
    dfset = dfset.sample(frac=1, replace=False)
...
#print(positives)
positives_shuf = positives.sample(frac=1, replace=False)
...
# # shuffle set n
n_shuf = n.sample(frac=1, replace=False)
...
history = model.fit(X_train_res, ytrainres_cat, epochs=30, batch_size=32, validation_data=(X_val_res, yvalres_cat),callbacks=[tensorboard_callback])#[EarlyStopping(monitor='loss',patience=7, min_delta=0.0001)])
...
identifier = 'contrastv3_150batch_size32KLSTM64epoch100_train0.6_2_dimension_data3_5_8_1' + datetime.now().strftime("%Y%m%d-%H%M%S")
